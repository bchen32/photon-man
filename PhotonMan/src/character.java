...
public class character extends object{
	...
	// stats
	boolean hasJumped1; boolean hasJumped2;
	...
	public character(sprites sp, int x, int y, boolean id) {
		...
		hasJumped1 = false; hasJumped2 = false;
	...
	@Override
	public void move() {
		if(isDead) {x += dx; return;}
		// if dead, continue to move but nothing else
		count++;
		if(count <= 10) currsp = mysp[1];
		if(count > 10) currsp = mysp[2];
		if(count > 20) currsp = mysp[3];
		if(count > 30) currsp = mysp[2];
		if(count > 40) {currsp = mysp[1]; count = 0;}
